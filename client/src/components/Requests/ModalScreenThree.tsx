...
const ModalScreenThree: React.FC<ModalScreenThreeProps> = ({ isOpen, onClose, children }) => {
    ...
    const [gifteeID, setGifteeID] = useState<number>(0)
    ...
    const handleColorChange = (selectedColors: string[]) => {
        setGiftee((prevGiftee) => ({ ...prevGiftee, Colors: selectedColors }));
    };

    const handleNameChange = (selectedName: string) => {
        console.log(`Selected Name: ${selectedName}`);
        setGiftee((prevGiftee) => ({ ...prevGiftee, GifteeName: selectedName }));
    };

    const handleRelationshipChange = (selectedRelationship: string) => {
        console.log(`Selected Relationship: ${selectedRelationship}`);
        setGiftee((prevGiftee) => ({ ...prevGiftee, CustomerRelationship: selectedRelationship }));
    };
    ...
    const makeGiftee = async () => {
        try {
            const response = await axios.post("/api/addGiftee", {
                Age: giftee.Age,
                Colors: giftee.Colors,
                CustomerID: 1,
                CustomerRelationship: giftee.CustomerRelationship,
                Gender: giftee.Gender,
                GiftRequests: giftee.GiftRequests,
                GifteeName: giftee.GifteeName,
                Interests: giftee.Interests
            });
            setGifteeID(response.data.ID)
